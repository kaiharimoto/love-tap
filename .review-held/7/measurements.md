...
## Still open
- **13 carries no `noor writing…`, and the reason I first wrote down was wrong.** 13's awaitView
  step read 9111 ms, and I put that down as a frame taking nine seconds to cross a loopback
  socket, with the sync engine's blob walk as the cause. Both halves are wrong, measured:
  - a probe on a freshly paired phone — same build, same far phone, sampling the app's own report
    — notices the frame after **261 ms**, and holds the indicator lit for the twenty seconds it
    was watched, with a sync round every 3.5 s;
  - `missingBlobs()` over 14,061 events costs **1-5 ms**, and `pushable` 0 ms;
  - and a fresh mask composes in **16.1 ms**, cached 0.011 ms — so neither is the build spike
    either.
  The 9111 ms is the harness's own cost: `__deskReport` on the thread runs the hero-framing search
  over 96 framings, so `awaitView` polls slowly. What stands: 13's picture has no typing indicator,
  its record read at the shutter says so, and I cannot yet say why the lapse ran out in that scene
  and not in the probe. **And the build spikes, found the same way:** 187 of 777
  frames of 11's scroll cost more than 400 ms to build (p95 773, max 1109) against a raster that
  never leaves 117-200 ms. Not the sync round and not the projector: it is the tear mask being
  baked into an image, once per note, on the build thread. `SlicedMasks.at` composes a nine-patch
  into a picture and calls `toImageSync` — 16.1 ms on the Dart VM, hundreds of milliseconds in
  CanvasKit. Measured by building the app with the mask taken out of the piece and scrolling the
  same anchor:

  | build | frames over 400 ms | build p95 | build max |
  |---|---|---|---|
  | as captured | 52 of 189 | 807 ms | 2142 ms |
  | with no tear mask | **4 of 146** | **34 ms** | 2514 ms |

  The fix is not to bake it: `drawImageNine` straight into the mask layer in `dstIn` is the same
  nine-patch without the offscreen image, and it is the first thing in the next cycle.
