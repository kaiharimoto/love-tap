...
void main() {
  ...
  setUpAll(() async {
    ...
    try {
      ...
      absent =
          'this bundle was packed without a material library '
  ...
  Future<Rect> open(WidgetTester tester, Size frame) async {
      ...
      AppScope.provide(
        ...
        child: MaterialApp(
          ...
          home: Scaffold(
            body: Column(
              children: [
                // At 360 wide the strip's own dials row overflows by 107 points, which is not this
                // sheet's and is filed as its own item (the-partner-strip-overflows-at-360-wide); a
                // stand-in of the strip's height stands there so this test measures the sheet.
                if (frame.width < 400)
  ...
  // Both frames the scenes are captured in: 01-05 and 13 at 480x1040, 06-08 and 15 at 360x780.
  // The picker was written against the first and 07_feeling_landing is the second; at 360 wide
  // the first version of this sheet had half the vocabulary scrolled off its top.
  for (final frame in const [Size(480, 1040), Size(360, 780)]) {
    testWidgets('at $frame the open corner is an opaque torn sheet over their card, not a tint', (
      tester,
    ) async {
      if (absent != null) return markTestSkipped(absent!);
      await open(tester, frame);
      ...
      for (final t in says) {
        for (final e
            in find.descendant(of: find.byType(PulseRegion), matching: find.text(t)).evaluate()) {
          checked++;
          final r = tester.getRect(find.byWidget(e.widget).first);
          expect(
            sheetRect.contains(r.topLeft) && sheetRect.contains(r.bottomRight),
            isTrue,
      ...
      final grounds = tester.widgetList<DecoratedBox>(
        find.descendant(of: find.byType(FeelingCorner), matching: find.byType(DecoratedBox)),
      );
      for (final g in grounds) {
        final d = g.decoration;
        expect(
          d is BoxDecoration && d.gradient != null,
          isFalse,
    ...
    testWidgets(
      'at $frame every feeling is on the sheet, and every family has its objects beside it',
      (tester) async {
        if (absent != null) return markTestSkipped(absent!);
        final screen = await open(tester, frame);
        ...
        final sheetFinder = find.byWidgetPredicate(
          (w) => w is PaperPiece && w.id == 'feelings.sheet',
        );
        ...
        expect(
          ...
          isEmpty,
          reason:
              '${missing.length} of ${active.length} feelings are not on the open sheet: $missing',
        );
        ...
        final tabs = [
          for (final f in Family.values)
            tester.getRect(
              find.byWidgetPredicate((w) => w is PaperPiece && w.id == 'family_${f.name}'),
            ),
        ];
        expect(tabs.map((r) => r.top.round()).toSet().length, greaterThan(1));
        final tabArea = tabs.fold<double>(0, (a, r) => a + r.width * r.height);
        expect(
          tabArea / (screen.width * screen.height),
          lessThanOrEqualTo(0.25),
          reason:
              'the family tabs take ${(100 * tabArea / (screen.width * screen.height)).round()}% '
              'of the frame',
        );
        for (final f in Family.values) {
