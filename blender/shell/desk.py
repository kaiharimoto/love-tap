...
def _placed_rays(h, w, seed, count=340):
    """Medullary rays as placed strokes: short, pale, lying across the rings, wrapped along the
    board so the strip tiles. Their positions are drawn, not generated by a frequency, which is the
    whole point — a sum of sinusoids has a spectrum and a run of rays does not.

    Lens-shaped, and wider than they were. A material critic has reported "one-pixel bright rules
    drawn across the desk" in nine of ten stills for four cycles — up to 440 px long at a mean
    ridge of +34.9 grey — and three hypotheses about where they came from were wrong. They are
    these: a ray was a Gaussian one to three pixels across and up to sixty long, with hard ends,
    which is a scratch and not a fleck. Ray fleck in quarter-sawn oak is a lens a few millimetres
    long and a fraction of one across, pale at its middle and fading to nothing at both ends and
    both sides. Three to seven pixels across, faded across as well as along, and fewer of them.
    """
    rng = np.random.default_rng(seed)
    v = np.linspace(0, 1, h, endpoint=False)[:, None]
    u = np.linspace(0, 1, w, endpoint=False)[None, :]
    ...
    for _ in range(count):
        y0 = rng.uniform(0, 1)
        x0 = rng.uniform(-0.05, 1.05)
        # across the rings (mostly along the board's width), leaning a little
        lean = rng.uniform(-0.22, 0.22)
        length = rng.uniform(0.012, 0.050)          # in board widths
        width = rng.uniform(0.0009, 0.0022)         # in board lengths
        along = (u - x0) + (np.mod(v - y0 + 0.5, 1.0) - 0.5) * lean / aspect
        across = (np.mod(v - y0 + 0.5, 1.0) - 0.5) - (u - x0) * lean * aspect
        # a lens: fading to nothing at both ends, not stopping at them
        taper = np.clip(1.0 - (np.abs(along) / length) ** 2, 0, 1) ** 1.6
        field += np.exp(-(across / width) ** 2) * taper * rng.uniform(0.25, 1.0) ** 1.7
    m = field.max() or 1.0
...
def board_grain(h, w, seed, rings=17.0, cant=0.10):
    ...
    #
    # These were a product of sinusoids along the board and across it, which is a grid however many
    # harmonics it is given. They are placed now: each ray is a short pale stroke at its own drawn
    # position, wrapped along the board so the strip still meets itself top to bottom. A ray is a
    # ray because of where a branch trace happened to cross the cut, so there is no frequency in
    # it for a ruler to find — which is the point, and it is also the better model.
    #
    # WHAT IS NOT KNOWN. A material critic reported four bright hairlines at a pitch of exactly 67
    # pixels in the desk of 02_chat.png, and three more runs at other pitches, and read the whole
    # desk as a tiled texture. That measurement has not been reproduced here: neither the
    # autocorrelation of the render's row means (2.3 against a noise floor, and 3.0 after this
    # change, so no better) nor a bright-row search in the desk strip of the artifact itself (ten
    # bright rows, gaps 3, 20, 13, 45, 4 — no clustering) finds a repeat. So this change is made
    # on its own merits and is NOT evidence that the finding is closed. The next cycle's material
    # critic is the test; if the hairlines are reported again, the thing to ask for is the exact
    # columns and threshold, because the disagreement is about method, not about the pixels.
    fleck = _placed_rays(h, w, seed + 2)
